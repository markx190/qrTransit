<template>
  <q-page>
    ...
    <div class="q-pa-md">
      Total Pasengers: {{ qrSales.length }}
      <q-table
        title="MANIFEST"
        :rows="qrSales"
        :columns="columns"
      ...
      >
        <template v-slot:body-cell-actions="props">
          <q-td class="text-uppercase" :props="props">
            <q-avatar>
              <img :src="props.row.user.photo_url" />
            </q-avatar>
            {{ props.row.user.name }}
          </q-td>
        </template>
        ...
        <template v-slot:body-cell-vehicle="props">
          <q-td class="text-uppercase" :props="props">
            {{ props.row.vehicle ? props.row.vehicle.vehicleId : "" }}
          </q-td>
        </template>
        ...
        <template v-slot:body-cell-dateOnBoarded="props">
          <q-td class="text-uppercase" :props="props">
            {{ props.row.createdAt }}
          </q-td>
        </template>
        <template v-slot:body-cell-embarkedDate="props">
          <q-td class="text-uppercase" :props="props">
            {{
              props.row.status === "ticketgenerated"
                ? "-"
                : props.row.embarkedDate
            }}
          </q-td>
        </template>
        <template v-slot:body-cell-disembarked="props">
          <q-td class="text-uppercase" :props="props">
            {{
              props.row.status === "ticketgenerated" ||
              props.row.status === "embarked"
                ? "-"
                : props.row.updatedAt
            }}
          </q-td>
...
export default defineComponent({
  ...
  computed: {
    ...mapGetters({
      ...
      qrManifestForExports: "qrTrans/qrManifestForExports",
    }),
  },
  methods: {
    checkIsLogged() {
    ...
    async getSales() {
      ...
      const passengers = await this.$store.dispatch("qrTrans/getQrSales", data);
      this.loading = false;
    ...
    async exportThis() {
      ...
      const pExports = await this.$store.dispatch(
        "qrTrans/getQrManifestForExports",
        auth
      );
      this.loading = false;
      console.log("export: ", pExports);
    },
    exportTable() {
      // naive encoding to csv format
      const content = [columnsZ.map((col) => wrapCsvValue(col.label))]
        .concat(
...
const columns = [
  ...
  },
  {
    name: "disembarked",
    align: "left",
    label: "DATE DISEMBARKED",
    field: "dateDisembarked",
    sortable: false,
  },
];
