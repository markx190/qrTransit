<template>
  <q-dialog v-if="dDialog" :model-value="true">
      ...
      <q-card-section>
        ...
        <div class="row bg-grey-2 rounded-borders">
          <div class="col-md-8 q-pa-md" style="max-width: 1350px">
            <div v-for="destination in destinationsData" :key="destination">
              {{ destination }}
            </div>
          </div>
        </div>
...
export default defineComponent({
  name: "ShowDestinationsDialog",
  props: ["destinationsData", "dDialog", "fTitle"],
  data() {
    return {
      salesOrigins: [],
    };
